...
Future<List<Division>> loadDivisions() async {
  final String response = await rootBundle.loadString('assets/data/bd-divisions.json');
  final data = await json.decode(response);
  return parseDivisions(data);
}

Future<List<District>> loadDistricts() async {
  final String response = await rootBundle.loadString('assets/data/bd-districts.json');
  final data = await json.decode(response);
  return parseDistricts(data);
}

Future<List<Upazila>> loadUpazilas() async {
  final String response = await rootBundle.loadString('assets/data/bd-upazilas.json');
  final data = await json.decode(response);
  return parseUpazilas(data);
}

Future<List<Postcode>> loadPostcodes() async {
  final String response = await rootBundle.loadString('assets/data/bd-postcodes.json');
  final data = await json.decode(response);
